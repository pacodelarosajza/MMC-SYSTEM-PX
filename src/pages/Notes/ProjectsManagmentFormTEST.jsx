...
const ProjectsManagmentForm = () => {
  ...
  const [isModalErrorProjectFieldsOpen, setIsModalErrorProjectFieldsOpen] = useState(false);
  const [isModalErrorUserAdminFieldOpen, setIsModalErrorUserAdminFieldOpen] = useState(false);
  const [isModalErrorUserOperFieldOpen, setIsModalErrorUserOperFieldOpen] = useState(false);
  const [isModalErrorAssemblyFieldOpen, setIsModalErrorAssemblyFieldOpen] = useState(false);
  ...
  const handleCreateProject = async (event) => {
    ...
    if (!newProject.identification_number || !newProject.cost_material || !newProject.delivery_date) {
    ...
    try {
      ...
      if (InTable.length === 0) {
        setIsModalErrorAssemblyFieldOpen(true);     //OPTIMIZE: ME QUEDE EN EL CONTROK DE ERRORES ya deiodo de haber quedado, toca seguir con subensambles
        return;
      }
      ...
      const subassemblyPromises = InTableSubass.map((rowSubass) =>
        axios.post(`${apiIpAddress}/api/postAssembly`, {
          assembly_id: rowSubass.assemblyId,
      ...
      {showCard && (
        <div className="bg-gray-800 px-5 rounded-lg shadow-lg mb-5">
          <div className="px-2 pb-5">
            ...
            {/* OPTIMIZE: REGISTRATION ASSEMBLIES */}
            <div className="col-span-full pt-3">
              <div className="mb-5">
                <h2 className="text-xl text-gray-200 font-bold">
          ...
          <Modal
            ...
            title="Required fields"
          >
            <p>Please, Select at least one personnel with reception authorization.</p>
          </Modal>
          ...
          <Modal
            ...
            title="No assemblies"
          >
            <p>Assembly data is needed to continue with the project registration.</p>
          </Modal>
